...
#import {
  ...
  #badRequestError
#} from '/imports/helpers/webApiHelper.coffee'
...
JsonRoutes.createCrudRoutes = (collection) ->
  camelizedCollectionName = pluralize(_.camelCase(collection.name))
  crudRoutes = [
    {
      method: 'get'
      path: '/' + camelizedCollectionName
      callback: (req, res, next) ->
        entities = undefined
        ...
        if entities
          ...
          resourceNotFoundError req, res, next
        return
    ...
    }
    {
      ...
      callback: (req, res, next) ->
        entity = undefined
        entityId = undefined
        entityId = collection.insert(req.bodyParams)
    ...
    {
      method: 'get'
      path: '/' + camelizedCollectionName + '/:id'
      callback: (req, res, next) ->
        entity = undefined
    ...
    }
    {
      ...
      callback: (req, res, next) ->
        entity = undefined
        entityIsUpdated = undefined
        entityIsUpdated = collection.update(req.params.id, req.bodyParams)
        if entityIsUpdated
          entity = collection.findOne(req.params.id)
          @sendResult res, data: entity
        else
  ...
  _.each crudRoutes, (crudRoute) ->
    JsonRoutes.add crudRoute.method, crudRoute.path, crudRoute.callback
    return
  return
